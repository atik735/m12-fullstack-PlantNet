    ...
    <form onSubmit={handleSubmit}>
      ...
      {cardError && <p className='text-red-500 mb-6'>{cardError}</p>}
        <div className='flex justify-between'>
              <button className='px-3 py-1 bg-green-400 rounded cursor-pointer' type="submit" disabled={!stripe || processing}>
        {processing ?<ClipLoader size={24} /> : `Pay ${totalPrice}$`}
      </button>
      <button onClick={closeModal} className='px-3 py-1 bg-green-400 rounded cursor-pointer' type="button" >
        Cancel
      </button>
        </div>
    </form>
